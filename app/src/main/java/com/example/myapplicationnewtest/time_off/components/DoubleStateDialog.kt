...
fun DoubleStateDialog(
    ...
    val hasRefuseAndDraft = leaveRecords.any { it.state == "refuse" } &&
            leaveRecords.any { it.state == "draft" }
        ...
        confirmButton = {
            Button(
                ...
                colors = ButtonDefaults.buttonColors(
                    containerColor = if (hasRefuseAndDraft) MaterialTheme.colorScheme.tertiary else MaterialTheme.colorScheme.secondary ,
                ...
                Column {
                    leaveRecords.forEach { record ->
                        ...
                        if (showDeleteConfirmation) {
                            val draftRecord = leaveRecords.find { it.state == "draft" }
                            if (draftRecord != null) {
                                    ...
                                    onConfirmDelete = {
                                        CoroutineScope(Dispatchers.IO).launch {
                                            val request = TimeOffRequestForRequestEmployee(
                                                ...
                                                date_from = draftRecord.start_date,
                                                date_to = draftRecord.end_date,
                                                leave_type = 6,
